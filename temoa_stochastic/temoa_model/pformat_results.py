...
def dat_to_db(input_file, output_schema, run_partial=False):
	...
	for i in parsed_data['tech_baseload']:
		if i is '':
			continue
		output_schema.execute("INSERT OR REPLACE INTO technologies VALUES('"+i+"', 'pb', '', '', '');") # JAB 2019-09-25: Added 5th column
	for i in parsed_data['tech_storage']:
		if i is '':
			continue
		output_schema.execute("INSERT OR REPLACE INTO technologies VALUES('"+i+"', 'ps', '', '', '');")# JAB 2019-09-25: Added 5th column
	for i in parsed_data['tech_hourlystorage']:
		if i is '':
			continue
		output_schema.execute("INSERT OR REPLACE INTO technologies VALUES('"+i+"', 'ph', '', '', '');")		# JAB 2019-09-25: Added 5th column
	for i in parsed_data['tech_production']:
		...
		if i in parsed_data['tech_baseload']:
			continue
		output_schema.execute("INSERT OR REPLACE INTO technologies VALUES('"+i+"', 'p', '', '', '');")# JAB 2019-09-25: Added 5th column
	for i in parsed_data['tech_resource']:
		if i is '':
			continue
		output_schema.execute("INSERT OR REPLACE INTO technologies VALUES('"+i+"', 'r', '', '', '');") # JAB 2019-09-25: Added 5th column
