...
localTables = ['plants_to_include','fuels_to_include','connections_to_include','include_baseload',
               ...
               'future_periods','active_future_periods','allTimePeriods','commodities']
...
def build(modelInputs,scenarioXLSX,scenarioName,outFilename,sensitivity={},MCinputs={}, path='.' ):
    ...
    if debug == True:
        print sensitivity
    if not len(sensitivity)==0: # if dictionary is empty, it will evaluae to false, and no senstivity will be performed
...
def processSystem(inputs,local,outputs):
    ...
    # SegFrac
    for season, season_frac in zip(inputs['representativeDays'].representativeDay,inputs['representativeDays'].timeFrac):
    ...
    for season, season_frac in zip(inputs['representativeDays'].representativeDay,inputs['representativeDays'].dmdFrac):
...
#    inputs['Connections']               = inputs['Connections'].set_index('connection')   
    ...
    for techType in local['plants_to_include']:
        ...
        # Constraints
        tech['max_capacity'] = inputs['PowerPlantsConstraints'].loc[techType,'MaxCapacity']/1000.0 # Convert from MW to GW
...
#    inputs['Fuels']  = inputs['Fuels'].set_index('fuel')
    ...
    for techType in local['fuels_to_include']:
        ...
        tech['c2a']         = 'N' # Indicator whether to include a capacity to activity input, only needed for powerplants
...
def processTech(inputs,local,outputs,tech):
    ...
    # --------
        ...
        if tech['sector'] == 'electric': # Only apply to powerplant technologies
            ...
            if not tech['fuel'] in list(inputs['capacityFactorTOD'].fuel):
                for representativeDay in inputs['representativeDays'].representativeDay:
                    for timeOfDay in inputs['timesOfDay'].timeOfDay:
                        outputs['CapacityFactorTech'].append((representativeDay, timeOfDay, tech['name'], tech['capacity_factor'], " " ))
            # Capacity factor that varies with timeOfday and representativeDay
                ...
                for index,row in inputs['capacityFactorTOD'].iterrows():
                    if row['fuel'] == tech['fuel']:
                        outputs['CapacityFactorTech'].append((row['representativeDay'], row['timeOfDay'], tech['name'], row['capacityFactor']*tech['capacity_factor'], " " ))
            ...
            for period in local['active_future_periods']:   
                for vintage in buildYears:    
                    if (vintage <= period) and ((period - vintage) < tech['lifetime']):
                        outputs['CostFixed'].append((str(period), tech['name'], str(vintage), tech['cost_fixed'], "M$/GW", " "))
        ...
        if goodValue(tech['efficiency']):
            for vintage in buildYears:
                outputs['Efficiency'].append((tech['fuel'], tech['name'], str(vintage), tech['output'], tech['efficiency'], " "))
    
        # EmissionActivity
        if goodValue(tech['emission_activity']):
            for vintage in buildYears:
                outputs['EmissionActivity'].append((emission_type, tech['fuel'], tech['name'], str(vintage), tech['output'], tech['emission_activity'], "kt/PJout", " "))
        ...
        if goodValue(tech['lifetime']):
            ...
            outputs['LifetimeLoanTech'].append((tech['name'], float(tech['lifetime']), " "))
        ...
        if tech['newBuilds'] =='Y' and tech['sector'] == 'electric' and local['include_growth_limit'] == "Y":
            ...
            if goodValue(local['MaxGrowthRate']):
                outputs['GrowthRateMax'].append((tech['name'],str(local['MaxGrowthRate']/100.0), 'Global Growth Limit - Fraction') )
...
#=============================================================================
def createSensitivityCases(scenarioXLSX, scenarioName, sensitivityInputs,multiplier):
...
def createMonteCarloCases(scenarioXLSX, scenarioName, sensitivityInputs,multiplier,n_cases=100):
    ...
    # Globals
    for var in params['global_vars']:
        df.loc[count] = ['Globals',var,'global']+np.random.triangular(-1.*multiplier,0,multiplier,size=n_cases).tolist()
        count = count + 1
    
    # PowerPlants
    for var in params['plant_vars']:
        for tech in params['plants']:
            df.loc[count] = ['PowerPlants',var,tech]+np.random.triangular(-1.*multiplier,0,multiplier,size=n_cases).tolist()
            count = count + 1
            
    # Fuels
    for var in params['fuel_vars']:
        for tech in params['fuels']:
            df.loc[count] = ['Fuels',var,tech]+np.random.triangular(-1.*multiplier,0,multiplier,size=n_cases).tolist()
            count = count + 1
            
    # Connections
    for var in params['conn_vars']:
        for tech in params['connections']:
            df.loc[count] = ['Connections',var,tech]+np.random.triangular(-1.*multiplier,0,multiplier,size=n_cases).tolist()
            count = count + 1
