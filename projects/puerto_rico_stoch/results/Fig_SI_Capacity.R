...
# Load data:
df1 <- read.csv("capacity_by_fuel_toPlot.csv")
df2 <- read.csv("capacity_by_tech_toPlot.csv")
df3 <- read.csv("capacity_by_tech_toPlot.csv")
...
# add unique value for each row - so that we keep all MC outputs after applying exclude
# df1$entry <- seq.int(nrow(df1))
# df2$entry <- seq.int(nrow(df2))
# df3$entry <- seq.int(nrow(df3))
...
levels <- c("Pre-2016",
            ...
            "2031-35",
            "2036-40")
df_smry$Year <- factor(df_smry$Year, levels = levels)
...
ggplot(df_smry,aes(x=Year, y=mean, ymin=min, ymax=max, fill=Case, group=Case, color=Case))+
  ...
  scale_fill_manual(values=cbPalette, labels=expression('No policy - No storms',
                                                        ...
                                                        'US$'~100~t^-1~CO[2]~'- Historical storm frequency',
                                                        'US$'~100~t^-1~CO[2]~' - Increased storm frequency'),guide = guide_legend(nrow = 3, label.hjust = 0))+
  labs(x='Year', y=expression(paste("New capacity (GW y"^-1,")")))+
...
# Save
ggsave('Fig_SI_capacity_overview_line.png', device="png",
       width=7.4, height=7.4, units="in",dpi=1000)
...
ggplot(df_smry,aes(x=Year, y=mean, ymin=min, ymax=max, fill=Case, group=Case, color=Case))+
  ...
  geom_errorbar(width=0.2, position=position_dodge(width=dodge))+
  scale_color_manual(values=cbPalette, labels=expression('No policy - No storms',
                                                         'No policy - Historical storm frequency',
                                                         'No policy - Increased storm frequency',
                                                         'RPS - No storms',
...
# Save
ggsave('Fig_SI_capacity_overview_bar.png', device="png",
       width=7.4, height=7.4, units="in",dpi=1000)
...
# data
#==============================
write.csv(df_smry,"capacity_summary.csv")
...
rename <- c("'Coal + petroleum'"="Coal + petroleum",
            "'Natural gas'"="Natural gas",
            "'Solar'"="Solar",
            "'Wind'"="Wind",
            "'Transmission - overhead'"="Transmission - overhead",
            "'Transmission - buried'"="Transmission - buried",
            "'Distribution - overhead'"="Distribution - overhead",
            "'Distribution - buried'"="Distribution - buried",
            "'Centralised - fossil'"="Centralised - fossil",
            "'Centralised - renewable'"="Centralised - renewable",
            "'Distributed - fossil'"="Distributed - fossil",
            "'Distributed - renewable'"="Distributed - renewable")
df_smry <- transform(df_smry, Technology = rename[as.character(Technology)])

df_smrya <- df_smry[ which(df_smry$Analysis == "'Fuel use'"),]
df_smryb <- df_smry[ which(df_smry$Analysis == "'Transmission and distribution'"),]
df_smryc <- df_smry[ which(df_smry$Analysis == "'Grid architecture'"),]

dodge = 0.9
ggplot(df_smrya,aes(x=Year, y=mean, ymin=min, ymax=max, fill=Technology, group=Technology, color=Technology))+
  facet_wrap(~Case, labeller = label_parsed)+
  geom_col(position=position_dodge(width=dodge))+
  geom_errorbar(width=0.2, position=position_dodge(width=dodge),colour="black")+
  labs(x='Year', y=expression(paste("New capacity (GW y"^-1,")")))+
  theme(legend.position="bottom", legend.title = element_blank(),axis.text.x = element_text(angle = 90,vjust=0.5))

ggsave('Fig_SI_capacity_byCase_fuel.png', device="png",
       width=9.0, height=7.4, units="in",dpi=1000)

ggplot(df_smryb,aes(x=Year, y=mean, ymin=min, ymax=max, fill=Technology, group=Technology, color=Technology))+
  facet_wrap(~Case, labeller = label_parsed)+
  geom_col(position=position_dodge(width=dodge))+
  geom_errorbar(width=0.2, position=position_dodge(width=dodge),colour="black")+
  labs(x='Year', y=expression(paste("New capacity (GW y"^-1,")")))+
...
ggsave('Fig_SI_capacity_byCase_TD.png', device="png",
       width=9.0, height=7.4, units="in",dpi=1000)

ggplot(df_smryc,aes(x=Year, y=mean, ymin=min, ymax=max, fill=Technology, group=Technology, color=Technology))+
  facet_wrap(~Case, labeller = label_parsed)+
  geom_col(position=position_dodge(width=dodge))+
  geom_errorbar(width=0.2, position=position_dodge(width=dodge),colour="black")+
  labs(x='Year', y=expression(paste("New capacity (GW y"^-1,")")))+
...
ggsave('Fig_SI_capacity_byCase_gridArch.png', device="png",
       width=9.0, height=7.4, units="in",dpi=1000)
